...
void draw_rectangle(vector<uint32_t> &img, const size_t img_w, const size_t img_h, const size_t x, const size_t y, const size_t w, const size_t h, const uint32_t color) {
    ...
    for (size_t i=0; i<w; i++) {
        for (size_t j=0; j<h; j++) {
            size_t cx = x+i;
            size_t cy = y+j;
            if(cx>=img_w || cy>=img_h)continue; // no need to check for negative values 
...
int main() {
    ...
    float player_x = 3.456; // player x pos
    float player_y = 2.356; // player y pos
    float player_a = 1.523; // player view direction
    const float fov = M_PI/3; // fov
    ...
    for(size_t i=0; i<win_w/2; i++){
        float angle = player_a-fov/2 + fov*i/float(win_w/2); // drawing the ray cone and the 3D view 
        for(float t=0; t<20; t+=.05){
            ...
            framebuffer[pix_x + pix_y*win_w] = pack_color(160,160,160); // draws visibility cone 

                if(map[int(cx)+int(cy)*map_w]!=' ') // when the ray touches a wall, then draw the vertical column to create 3D illusion
                    {
    ...
    // drawing the player on the map of the game
